...
BEGIN
  ...
  IF v_mode = 'selected' THEN
    IF v_audience ? 'user_ids' AND jsonb_array_length(v_audience->'user_ids') > 0 THEN
      SELECT array_agg(u::text::uuid) INTO v_user_ids
      FROM jsonb_array_elements_text(v_audience->'user_ids') u;
    ELSE
      RETURN jsonb_build_object('success', false, 'error', 'Selected mode requires audience.user_ids');
    END IF;
  END IF;
  ...
  LOOP
    ...
    ELSE
      INSERT INTO public.notifications (
        user_id, type, title, body, ref_id, metadata
      ) VALUES (
        v_user.id,
        'admin_broadcast',
        v_broadcast.title,
