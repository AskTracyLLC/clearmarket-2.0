CREATE OR REPLACE FUNCTION public.accept_territory_assignment(p_assignment_id uuid, p_rep_user_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_working_terms_request_id uuid;
  v_work_type text;
BEGIN
  -- Get rep_profile.id for this user (needed for rep_interest which references rep_profile.id)
  ...
  -- Get the post details (may be null)
  SELECT * INTO v_post
  FROM public.seeking_coverage_posts
  WHERE id = v_assignment.seeking_coverage_post_id;

  v_work_type := COALESCE(v_post.work_type, '');

  -- Build coverage/pricing summaries
  IF v_assignment.county_name IS NOT NULL AND btrim(v_assignment.county_name) <> '' THEN
  ...
  -- Update rep_interest status to connected
  ...
  -- ==========================================================
  -- Vendor ↔ Rep connection (✅ uses field_rep_id)
  -- ==========================================================
  ...
  -- ==========================================================
  -- Agreement summaries (for VendorMyReps display)
  -- ==========================================================
  ...
  -- ================================================================
  -- WORKING TERMS AUTO-CREATION (idempotent) ✅
  -- ================================================================
  SELECT id INTO v_working_terms_request_id
  ...
  IF v_working_terms_request_id IS NULL THEN
    INSERT INTO public.working_terms_requests (
      ...
      CASE
        WHEN v_assignment.county_name IS NOT NULL AND btrim(v_assignment.county_name) <> ''
          THEN ARRAY[v_assignment.county_name]
        ELSE NULL
      END,
      'Auto-created from territory assignment acceptance',
      'active',
      now(),
      now()
    )
    RETURNING id INTO v_working_terms_request_id;
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM public.working_terms_rows
    WHERE working_terms_request_id = v_working_terms_request_id
      AND vendor_id = v_assignment.vendor_id
      AND rep_id = v_assignment.rep_id
      AND state_code = v_assignment.state_code
      AND COALESCE(county_name, '') = COALESCE(NULLIF(btrim(v_assignment.county_name), ''), '')
      AND COALESCE(inspection_type, '') = COALESCE(NULLIF(btrim(v_work_type), ''), '')
      AND status = 'active'
  ) THEN
    INSERT INTO public.working_terms_rows (
      ...
      v_working_terms_request_id,
      v_assignment.vendor_id,
      v_assignment.rep_id,
      v_assignment.state_code,
      NULLIF(btrim(v_assignment.county_name), ''),
      NULLIF(btrim(v_work_type), ''),
      v_assignment.agreed_rate,
      NULL,
      'territory_assignment',
      true,
      COALESCE(v_assignment.effective_date::date, CURRENT_DATE),
      'active',
      now(),
      now()
    );
  ELSE
    UPDATE public.working_terms_rows
    SET
      rate = v_assignment.agreed_rate,
      effective_from = COALESCE(v_assignment.effective_date::date, CURRENT_DATE),
      included = true,
      status = 'active',
      updated_at = now(),
      inactivated_at = NULL,
  ...
  -- Mark rep's pending notification as read (leave your existing type)
  ...
  RETURN jsonb_build_object(
    ...
    'working_terms_request_id', v_working_terms_request_id
  );
END;
$function$;