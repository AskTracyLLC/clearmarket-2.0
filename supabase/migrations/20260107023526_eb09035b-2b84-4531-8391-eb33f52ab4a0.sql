...
BEGIN
  ...
  -- Get the rep_profile.id for this user (needed for rep_interest which references rep_profile, not profiles)
  ...
  IF v_assignment.seeking_coverage_post_id IS NOT NULL THEN
    ...
    -- NOTE: rep_interest.rep_id references rep_profile.id, not profiles.id
    IF v_rep_profile_id IS NOT NULL THEN
  ...
  -- Create notification for vendor (ref_id is uuid type, use uuid directly)
  ...
  -- Mark rep's pending notification as read (ref_id is uuid, compare uuid to uuid)
  UPDATE public.notifications
  SET is_read = true
  WHERE ref_id = p_assignment_id
